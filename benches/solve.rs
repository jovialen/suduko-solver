...
fn solve_easy(c: &mut Criterion) {
    ...
    easy.bench_function("1", |b| b.iter(|| suduko.clone().solve()));
    easy.finish();
}
...
fn solve_medium(c: &mut Criterion) {
    ...
    medium.bench_function("1", |b| b.iter(|| suduko.clone().solve()));
    medium.finish();
}
...
fn solve_hard(c: &mut Criterion) {
    ...
    hard.bench_function("1", |b| b.iter(|| suduko.clone().solve()));
    hard.finish();
}
...
fn solve_evil(c: &mut Criterion) {
    ...
    evil.sample_size(10);
    evil.bench_function("1", |b| b.iter(|| suduko.clone().solve()));
    evil.finish();
}
